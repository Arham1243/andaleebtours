<script>
    const hotelsDataPromise = Promise.all([
        fetch("{{ url('/') }}" + "/frontend/mocks/yalago_countries.json").then(r => r.json()),
        fetch("{{ url('/') }}" + "/frontend/mocks/yalago_provinces.json").then(r => r.json()),
        fetch("{{ url('/') }}" + "/frontend/mocks/yalago_locations.json").then(r => r.json())
