...
<div class="form-fields">
    <label class="title">{{ $label }} @if($required)<span class="text-danger">*</span>@endif</label>
    <div class="upload" data-upload>
        <div class="upload-box-wrapper">
            <div class="upload-box {{ !$existingImage ? 'show' : '' }}" data-upload-box>
                <input type="file" name="{{ $name }}" data-error="{{ $label }}"
                    id="{{ $name }}" class="upload-box__file d-none" accept="image/*"
                    data-file-input {{ $required ? 'required' : '' }}>
                <div class="upload-box__placeholder"><i class='bx bxs-image'></i>
                </div>
                <label for="{{ $name }}" class="upload-box__btn themeBtn">Upload
                    Image</label>
            </div>
            <div class="upload-box__img {{ $existingImage ? 'show' : 'd-none' }}" data-upload-img>
